...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.mapa = new mapboxgl.Map({
      ...
      center: [-79.050376, -2.4366067], // starting position
    ...
    if (!this._keyDat()) {
      this.getList.forEach(data => {
        if (data.cedula.endsWith(this.clave)) {
          ...
          this.isData = true;
          this.cargarMarcador(data);

        }
      });
    } else {
      this.getList.forEach(data => {
        if (data.clave.endsWith(this.clave)) {
          console.log(data);
  ...
  cargarMarcador(data: Catastro) {
    ...
    this.mapa.addControl(new mapboxgl.NavigationControl());
    let marker2;
    let latitud = (Number(data.latitud.replace("°", "")));
    let long = (Number(data.longuitud.replace("°", "")));
    marker2 = new mapboxgl.Marker({ color: 'black', rotation: 45 })
      .setLngLat([long, latitud])
      .addTo(this.mapa);
